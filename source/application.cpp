...
void application::set_config(unsigned int canale1, unsigned int canale2){
	...
	ch1=canale1;
	ch2=canale2;	
	for(unsigned int i =0; i<bins.size(); ++i) //potrei usare un append, ma come gestisco la situazione dello swap precedente?
...
void application::run(){
	...
	//	sleep(2); //aspetto un secondino che parta tutta la robaccia di root
		...
		if(what==3){
			std::cout<< std::endl << "ATTENZIONE: riceverai un sacco di insulti da ROOT ma va tutto ''bene''. Ciao!" << std::endl<< std::endl;
			sleep(2);
			stay_alive=false;
			processing=false;
		}
